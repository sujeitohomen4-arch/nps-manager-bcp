// ── CONSTANTES ──────────────────────────────────────────────────────────────
export const META_V  = 80;
export const META_P  = 82;
export const META_AG = 81;
...
// ── CÁLCULO NPS ──────────────────────────────────────────────────────────────
// NPS individual de un asesor o canal
...
// NPS de agencia = (NPS_Ventanilla + NPS_Plataforma) / 2  → peso 50/50 confirmado
export const calcAgencia = (asesores) => {
  const v = asesores.filter(a => a.canal === 'V');
  const p = asesores.filter(a => a.canal === 'P');
  const npsV = calcNPS(
    v.reduce((s, a) => s + a.prom, 0),
    v.reduce((s, a) => s + a.det,  0),
    v.reduce((s, a) => s + a.neut, 0)
  );
  const npsP = calcNPS(
    p.reduce((s, a) => s + a.prom, 0),
    p.reduce((s, a) => s + a.det,  0),
    p.reduce((s, a) => s + a.neut, 0)
  );
...
// Cuántas encuestas promotoras adicionales necesita para llegar a meta
...
// Cuánto sube el NPS de agencia si este asesor recibe +1 promotor
